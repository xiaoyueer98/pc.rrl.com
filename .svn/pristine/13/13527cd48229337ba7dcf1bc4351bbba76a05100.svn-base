...
function ajx(page, number) {
    if ((page - 1) * number >= count) {
        ...
        $.ajax({
            type: 'POST',
            url: '?s=/Home/Webchatpush/get_account_list',
            ...
            success: function (data) {
                ...
                $(result).each(function (k) {
                    ...
                    var incr = this.incr > 0 ? "+" + this.incr : this.incr;
                   
                    var str = "";
                    str = '<div class="resultjobs clearfix">'
                            + '<div class="job_title">'
                            + '<h3 class="colorhei">' + this.username + '</h3>'
                            + '</div>'
                            + '<div class="jobsInfo">'
                            + '<p>' + this.comment + '' + this.created_at + '</p>'
                            + '</div>'
                            + '<div class="enterpriseMore">' + incr + '</div>'
                            + '</div>';
                    $('#thelist2').append(str);
