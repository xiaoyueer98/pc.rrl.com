		...
		<ul class="pd">
			...
			<li class="pd_list mytj">
				...
				<ul style="display:block">
					...
					<li><a href="<?php echo U('Login/IsRecommended');?>"><em class="hre">正在推荐 </em></a></li>
				...
				<div class="recom1_tp">
					...
					<div class="tab">
						<table  border="1">
							...
							<?php if(is_array($comp)): $i = 0; $__LIST__ = $comp;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="bg">
								<td><?php echo ($vo["recoreid"]); ?></td>
								<td></td>
								<td><?php echo ($vo["cpname"]); ?></tb>
								<td><a href="<?php echo U('Login/EnterIndex2');?>"><?php echo ($vo["cascname"]); ?></a></td>
								<td>未审核</td>
								<td><?php echo ($vo["Tariff"]); ?></td>
