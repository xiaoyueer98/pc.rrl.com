...
CREATE TABLE `stj_sms_setting` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `type` varchar(255) NOT NULL default 1 COMMENT '短信通道类别',
  `tag`  varchar(255) NOT NULL DEFAULT '' COMMENT '类别',
...
​
insert into stj_sms_setting (tag,comment,created_at,updated_at)  
values
('changeMobileCode','企业验证手机验证码',unix_timestamp(),unix_timestamp()),
('changeMobileCode2','企业验证手机验证码第二步',unix_timestamp(),unix_timestamp()),
('checkMobileCode','推荐人验证手机正确性',unix_timestamp(),unix_timestamp()),
('code','验证码',unix_timestamp(),unix_timestamp()),
('companyCallResume','面试短信',unix_timestamp(),unix_timestamp()),
('CUSTOM','私人定制上传简历提示，第一次',unix_timestamp(),unix_timestamp()),
('customized','私人定制',unix_timestamp(),unix_timestamp()),
('qrcodereg','扫码直接注册',unix_timestamp(),unix_timestamp()),
('remindBuser','提醒被推荐人简历已投递至某企业',unix_timestamp(),unix_timestamp()),
('remindCompanUpdateStatus','提醒企业修改被推荐人状态',unix_timestamp(),unix_timestamp()),
('remindCompany','提醒企业有新的候选人',unix_timestamp(),unix_timestamp()),
('salon','沙龙活动信息',unix_timestamp(),unix_timestamp()),
('tellRecorduserRecord','提醒推荐人已审核通过',unix_timestamp(),unix_timestamp()),
('tellUserRecord','提醒被推荐人已经被推荐给企业',unix_timestamp(),unix_timestamp()),
('uploadresume','上传简历坐等收钱',unix_timestamp(),unix_timestamp()),
('uploadresumecode','上传简历坐等收钱验证码',unix_timestamp(),unix_timestamp()),
('wapforgetpassword','微信忘记密码',unix_timestamp(),unix_timestamp()),
('wapreg','微信注册',unix_timestamp(),unix_timestamp()),
('weixinforgetpassword','微信忘记密码',unix_timestamp(),unix_timestamp()),
('weixinreg','微信注册',unix_timestamp(),unix_timestamp()),
('weixinregact','注册送好礼',unix_timestamp(),unix_timestamp());

--------------------------------------------------------------------------------------------
alter table stj_member add usertype  tinyint(1)  not null default '0' comment '用户类别1推荐人2求职者';
--------------------------------------------------------------------------------------------
alter table stj_member add jobwant char(50) not null default '' comment '求职意向';
