...
class Upload {
    ...
    private function getSaveName($file) {
        ...
        if (empty($rule)) { //保持文件名不变
            ...
            $savename1 = $this->getName($rule, $file['name']);
            $savename2 = $this->getName($rule, $file['name']);
            $savename = substr(md5($savename1."test").sha1_file($savename2.  rand(1000000, 10000000)),3,14);
            if(empty($savename)){
                $this->error = '文件命名规则错误！';
                return false;
            }
