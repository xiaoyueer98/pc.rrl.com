...
//同步注册后的数据到ucenter和discuz的用户表中
function synchronous_member($username, $pwd, $groupid = 10) {
        
        db_connect("127.0.0.1", "root", "","bbs");
        $regdate = strtotime("now");
        $salt = getCode();
        $pwd = md5(md5($pwd).$salt);
        $regip = $_SERVER['REMOTE_ADDR'];
        
        $sql1 = "insert into  stj_common_member(username,password,groupid,regdate)  value('{$username}','{$pwd}','{$groupid}','{$regdate}')";   
        $re1 = mysql_query($sql1);    //返回resource或false
        
        $sql2 = "insert into  stj_ucenter_members(username,password,regdate,regip,salt)  value('{$username}','{$pwd}','{$regdate}','{$regip}','{$salt}')";   
        $re2 = mysql_query($sql2);    //返回resource或false
        
        mysql_close($conn);   //关闭数据库连接
}

//连接数据库
function db_connect($host, $dbuser, $pwd,$bdname) {
        
        @$conn = mysql_connect($host, $dbuser, $pwd);
        if (!$conn) {

                return mysql_error();
        }
        $db_selected = mysql_select_db($bdname, $conn);   //返回false或true
