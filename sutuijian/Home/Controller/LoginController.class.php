...
class LoginController extends Controller {
        ...
        public function changeTelephoneStep1() {
                ...
                if (!$_SESSION['leveltime']) {
                        ...
                        $result = smsChannel($telephone, $content, 0, "changeMobileCode", "推荐人验证手机验证码");
                        if ($result['code'] == "200") {
        ...
        public function changeTelephoneStep2() {
                ...
                if (!$_SESSION['leveltime2']) {
                        ...
                        $result = smsChannel($telephone, $content, 0, "changeMobileCode2", "推荐人验证手机验证码第二步");
                        if ($result['code'] == "200") {
...
//修改已验证的手机号
        public function checktelephonecode() {
                ...
                if (!$_SESSION['leveltime3']) {
                        ...
                        $result = smsChannel($telephone, $content, 0, "checkMobileCode", "推荐人验证手机正确性");
                        if ($result['code'] == "200") {
        ...
        function Recommended() {
                ...
                if (!empty($act)) {
                        ...
                        if ($id > 0) {
                                ...
                                //如果是通过精简模式上传的简历，且信息还没有完善，则显示时将未填写的内容隐藏
                                if (($resumeInfo['workexper'] && $resumeInfo['education'] && $resumeInfo['because'] || $resumeInfo['wordname'] && $resumeInfo['because'])  && empty($resumeInfo['match_company']) && empty($resumeInfo['match_educational']) && empty($resumeInfo['match_treatment']) && empty($resumeInfo['match_skill'])) {
                                        $this->assign("upload_resume_set", "1"); //为1则为上传简历模式，只有姓名、电话、上传简历等模块
                                }
                                if($resumeInfo['age']!=="" && $resumeInfo['email'] ){
                                        $this->assign("upload_resume_rec", "1"); //为1则为推荐时上传的简历
                                }
                        } else {
                                $type = 1;
                                $this->assign("upload_resume", "1"); //为1则为上传简历模式，只有姓名、电话、上传简历等模块
                        }
                        $isNewUser = "保存";
        ...
        public function btjadd() {
                ...
                $data['sex'] = I('sex');
                $data['age'] = I('age')?I('age'):"0";
                $data['state'] = I('state');
                $data['mobile'] = I('mobile');
                $data['email'] = I('email')?I('email'):"";
                $data['qqnum'] = I('qqnum')?I('qqnum'):"";
                ...
                $data['match_industry'] = $_POST['match_industry'] ? $_POST['match_industry'] : "";
                $data['match_title'] = $_POST['match_title'] ? $_POST['match_title'] : "";
                $data['match_skill'] = $_POST['match_skill'] ? $_POST['match_skill'] : "";
                $data['keyword'] = $_POST['keyword'] ? $_POST['keyword'] : "";
                $data['posttime'] = time();
                $arAllResume = M("resume")->query("select *  from stj_resume where mobile='{$data['mobile']}' and isshow=1 ");
                
                if(!empty($arAllResume)){
                     $this->error("该候选人简历在系统中已存在");
                     exit();   
                }
                ...
                if (empty($id)) {
                        ...
                        if ($vo = $resume->create($data)) {
                                ...
                                if ($list !== false) {
                                        ...
                                        if ($isRecord && $_POST['keyword'] && $_POST['match_educational'] && $_POST['match_title'] && $_POST['match_skill']) {
                                                $arMatchResume = array();
...
//                            $arRecordInfo['username'] = $arResumeInfo['username'];
        ...
        function RecommendTheCandidate() {
                ...
                foreach ($arResumeList as &$arResumeInfo) {
                        ...
                        $arAllResume = M("resume")->query("select s.id,s.mobile bt_mobile  from stj_resume r join stj_resume s  on r.id={$arResumeInfo['id']} and s.mobile=r.mobile ");
                        $where = "";
...
//                        $isRecord = M("record")->where($where . " AND j_id='$jid' AND (audstart=1 or audstart=3 or audstart=5 or audstart=6) AND status !=1")->find();
                        $isRecord = M("record")->where($where . " AND j_id='$jid'")->find();
                        if ($isRecord) {
...
//                        $isRecord = M("record")->where("j_id='$data[j_id]' and bt_id='" . $data[bt_id][$i] . "' and (audstart=1 or audstart=3 or audstart=5 or audstart=6) and status !=1")->find();
        ...
        public function getCode() {
                ...
                $post_data = "account=cf_zpkj&password=renrenlie231&mobile=" . $mobile . "&content=" . rawurlencode("您的验证码是：" . $mobile_code . "。请不要把验证码泄露给其他人。");
