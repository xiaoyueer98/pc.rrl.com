 ...
 * @since       1.0 
 */
class WebchatcustomizedController extends Controller {
        ...
        function index() {
                ...
                if (empty($arShare)) {
                        $arShare['wxurl'] = "http://www.renrenlie.com/Home/Webchatcustomized/index.html";
                ...
                $this->display("Webchatnew/custom");
        }
...
//                echo "<pre>";var_dump($data);echo "</pre>";
                ...
                $memberOb = M("member");
                $arMember = $memberOb->query("select * from stj_member  m join stj_userinfo u where m.username=u.username and mobile='{$data['mobile']}' and flag=0");
                if (empty($arMember)) {

                        //创建用户
                        $userOb = M("userinfo");
                        $username = $this->create_username($userOb);
                        $pwd = substr($data['mobile'], 5);
                        ...
                        $comment = "私人定制";
                        $linkid = $memberId;
                        smsChannel($data['mobile'], $content, $linkid, $tag, $comment);
                } else {
