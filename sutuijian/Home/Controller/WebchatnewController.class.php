...
//                        $isThreeCompany = M("record")->query("select *  from stj_record  r join stj_job b on  r.j_id = b.id  and " . $where . "  and (audstart=1 or audstart=3 or audstart=5 or audstart=6) and status !=1 group by  b.cpid;");
        ...
        public function resume_time() {
                ...
                if (!empty($status)) {
                        header("location:/Home/Webchatnew/push_resume/jobid/" . $jobid);
                        die;
                }
...
//                $isThreeCompany = M("record")->query("select *  from stj_record  r join stj_job b on  r.j_id = b.id  and " . $where . "  and (audstart=1 or audstart=3 or audstart=5 or audstart=6) and status !=1 group by  b.cpid;");
        ...
        function qa() {
                ...
                if (empty($arShare)) {
                        $arShare['wxurl'] = "http://www.renrenlie.com/Home/Webchatnew/qa.html";
        ...
        function upresume_info() {
                ...
                if (empty($arShare)) {
                        $arShare['wxurl'] = "http://www.renrenlie.com/Home/Webchatnew/upresume_info.html";
