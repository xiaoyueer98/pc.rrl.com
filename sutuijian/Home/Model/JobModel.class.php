...
class JobModel extends Model {
        ...
        public function get_detail($id) {
                $return_data = $this->table('stj_job job')
                        ->field('job.title,job.report,job.report_number,job.type,job.employ,job.starttime,job.posttime,job.experience,job.jobplace,job.education,job.endtime,job.joblang,com.cpname,com.remark,job.softlytip,com.webname,job.email,job.mobile,com.address,com.stage,com.nature,com.scale,com.brightreg,com.intro,com.bright,job.content,job.workdesc,job.treatment,job.Tariff,job.Jobcate,job.strycate,cas.cascname,com.id as cpid,job.id')
                        ->join('stj_company com ON job.cpid = com.id ')
                        ->join('stj_casclist cas ON job.jobplace = cas.id  and  job.id=' . $id)
                        ->find();
